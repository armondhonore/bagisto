...
**Live:** [https://relaxed-weasel-bagisto.cloud.nexlayer.ai](https://relaxed-weasel-bagisto.cloud.nexlayer.ai)  

**Runtime:**  · **Port:** auto-detected · **Deploy branch:** 2.4

---

## How this deployment works

**bagisto** is deployed on [Nexlayer](https://nexlayer.ai) — a container-native
platform where every push to `2.4` triggers a fully automated build-and-deploy
...
application:
  ...
  pods:
  - name: app
    image: mirror.gcr.io/bagisto/bagisto:latest
    path: /
    servicePorts:
    - 80
    vars:
      ...
      DB_USERNAME: bagisto
      DB_PASSWORD: bagisto
      APP_URL: "https://relaxed-weasel-bagisto.cloud.nexlayer.ai"
      APP_KEY: "base64:BAGISTOkeyhere12345678XXXXXXXX="
  - name: mysql
...
Triggers on:
- **Push** to `2.4` → production redeploy
...
> `<podName>.pod:<port>` syntax. CI/CD runs on push to `2.4`."*
